...
def kb_admin_menu(auto_publish_enabled: bool = False) -> KeyboardBuilder:
    kb = KeyboardBuilder()
...
def kb_auto_publish_menu(auto_publish_enabled: bool) -> KeyboardBuilder:
    ...
    status = "✅ Включена" if auto_publish_enabled else "❌ Выключена"
    # В aiomax нет кнопок без callback, поэтому делаем обычную кнопку с пустым payload (или просто текст)
    # Можно использовать LinkButton с пустым URL или просто добавить как текст через add
    # Но лучше сделать callback, который ничего не делает, либо просто текст через отдельное сообщение.
    # В клавиатуре нельзя добавить просто текст, поэтому оставим кнопку с callback-заглушкой.
    kb.add(CallbackButton(f"Статус: {status}", "auto_publish_status", intent='default'))
    # Кнопка переключения
    if auto_publish_enabled:
        kb.row(CallbackButton("⏹ Выключить", "admin:auto_publish_toggle", intent='default'))
    else:
        kb.row(CallbackButton("▶️ Включить", "admin:auto_publish_toggle", intent='default'))
    # Кнопка возврата
    kb.row(CallbackButton("↩️ Назад", "admin:menu", intent='default'))
    return kb
