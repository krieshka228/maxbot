"""
db.py — Работа с базой данных (SQLAlchemy + async).

ВАЖНО: Max-бот и Telegram-бот используют ОДНУ И ТУ ЖЕ базу данных
(SQLite-файл, путь задаётся через DATABASE_URL в .env — должен совпадать
с DATABASE_URL Telegram-бота). Схема таблиц идентична схеме Telegram-бота
(см. telegram_bot/bot/db.py) — это обязательное условие, иначе боты не
смогут читать данные друг друга.

Max-бот НЕ синхронизирует товары из канала Telegram — он только ЧИТАЕТ
товары (Product), которые туда положил Telegram-бот, и публикует их в
канал Max. upsert_product() здесь сохраняется для будущей админ-правки
остатков из Max-бота (синхронизация остатков, не каталога).
"""
...
def _build_engine():
    """Создаёт async-движок. Для серверных СУБД включает пул соединений
    с pool_pre_ping (авто-переподключение) — см. telegram_bot/bot/db.py."""
...
async def dispose_engine() -> None:
    """Корректно закрывает пул соединений при graceful shutdown."""
...
def _ensure_added_columns(conn):
    ...
    if "products" in insp.get_table_names():
        ...
        for col in ["photo_file_ids", "video_file_ids", "max_photo_ids", "max_video_ids", "max_post_id"]:
            if col not in cols:
...
async def get_or_create_user(
    ...
    **kwargs,
    ...
    """Получает пользователя или создаёт, если его нет (с защитой от гонок)."""
    # 1. Пытаемся получить существующего пользователя
    user = await session.get(User, user_id)
    if user is not None:
        # Обновляем недостающие поля (не перезаписываем существующие)
        updated = False
        for key, value in kwargs.items():
            if value is not None and getattr(user, key, None) is None:
    ...
    try:
        ...
        # 3. Если INSERT упал из-за дубляжа (гонка) – откатываем и повторно получаем
        await session.rollback()
        user = await session.get(User, user_id)
        if user is None:
            # Если пользователь всё ещё не найден – поднимаем исключение
            raise
        return user
...
# ---------------------------------------------------------------------------
# Чтение каталога (с TTL-кэшем). Эти функции отсутствовали в исходном
# Max-боте — без них handlers/catalog.py не может ничего показать.
# ---------------------------------------------------------------------------

async def get_active_categories(session: AsyncSession) -> list[str]:
    """Список категорий, в которых есть активные товары (кэш TTL=60с)."""
    async def _load():
        stmt = (
...
async def get_active_products_in_category(session: AsyncSession, category: str) -> list[Product]:
    """Активные товары категории, отсортированы по id (кэш TTL=60с)."""
    async def _load():
        stmt = (
            select(Product)
...
async def get_all_active_products(session: AsyncSession) -> list[Product]:
    """Все активные товары (кэш TTL=60с)."""
    async def _load():
...
async def get_products_without_max_post(session: AsyncSession) -> list[Product]:
    """Активные товары, ещё не опубликованные в канал Max — для публикации."""
    stmt = (
        select(Product)
    ...
    logger.info(f"Найдено товаров для публикации: {len(products)}")
    return products
...
async def upsert_product(
    ...
    """Создаёт/обновляет товар по post_id."""
...
async def update_product_stock(session: AsyncSession, product: Product, stock: int) -> Product:
    """Изменение остатка из админки Max-бота (без полного upsert)."""
    product.stock = stock
    product.is_active = stock > 0 and product.in_stock
    await session.commit()
    await session.refresh(product)
    invalidate_catalog_cache()
    return product
