...
def build_monthly_report(orders: list, users: list) -> bytes:
    ...
    ws1.title = "Заказы"
    headers1 = [
        "№ заказа", "Username", "ФИО", "Телефон", "Способ доставки", "Адрес",
        "Товар", "Кол-во", "Цена, ₽", "Сумма, ₽", "Платформа", "Дата заказа",
    ]
    ...
    for order in orders:
        ...
        platform = user.platform if user and user.platform else "—"
        fio = order.full_name or (user.full_name if user else None) or "—"
        ...
        if not order.items:
            ...
            ws1.cell(row=row, column=11, value=platform).border = _thin_border()
            ws1.cell(row=row, column=12, value=date_str).border = _thin_border()
            row += 1
        else:
    ...
    headers2 = ["ID", "Username", "Имя", "Телефон", "Адрес", "Платформа", "Согласие ПД", "Дата регистрации"]
    ...
    for user in users:
        ...
        ws2.cell(row=row, column=6, value=user.platform or "—").border = _thin_border()
        ws2.cell(row=row, column=7, value="Да" if user.consented else "Нет").border = _thin_border()
        date_str = user.created_at.strftime("%d.%m.%Y") if user.created_at else "—"
        ws2.cell(row=row, column=8, value=date_str).border = _thin_border()
        row += 1
...
def build_clients_excel(users: list) -> bytes:
    ...
    headers = ["ID", "Username", "Имя", "Телефон", "Адрес", "Платформа", "Согласие ПД", "Дата регистрации"]
    ...
    for user in users:
        ...
        ws.cell(row=row, column=6, value=user.platform or "—").border = _thin_border()
        ws.cell(row=row, column=7, value="Да" if user.consented else "Нет").border = _thin_border()
        date_str = user.created_at.strftime("%d.%m.%Y") if user.created_at else "—"
        ws.cell(row=row, column=8, value=date_str).border = _thin_border()
        row += 1
